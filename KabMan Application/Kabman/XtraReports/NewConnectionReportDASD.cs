...
{
    public partial class NewConnectionReportDASD : DevExpress.XtraReports.UI.XtraReport
    {
        public NewConnectionReportDASD()
        {
            InitializeComponent();
        }
        ...
        {
            DataSet dstmp = DBAssistant.ExecProcedure("[sproc].[ConnectionDetail_Select_ByConnectionDevice_Detail]", new object[] { "@DeviceID", 1 });
            //DataSet dstmp = DBAssistant.ExecProcedure("[sproc].[ConnectionDetail_Select_ByConnectionDevice_Detail_ByID]", new object[] { "@ConnID", newConnectionId[0], "@DeviceID", 1 });
            ...
            for (int i = 0; i < dstmp.Tables[0].DefaultView.Count; i++)
            {
                ...
                dr["Port"] = dstmp.Tables[0].Rows[i]["Port"];
